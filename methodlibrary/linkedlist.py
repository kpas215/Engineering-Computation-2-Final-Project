...
class linkedlist:
    ...
    #removes a value from anywhere in the doubly linked list (yeah im chill like that)
    def remove(self, value):
        #search through the list to see if the value can be found
        curr = self.__head
        while curr is not None:
            if curr.get_data() == value:
                #we found it! lets get lit
                
                #if the node is located at the head, we can simply just pop the head
                if curr is self.__head:
                    return self.pop_head()

                #if the node is located at the tail, we can simply pop the tail
                if curr is self.__tail:
                    return self.pop_tail()
                
                #if its not at the head or at the tail, we know its at the middle
                
                prevnode = curr.get_previous()
                nextnode = curr.get_next()

                if not isinstance(prevnode, doublenode):
                    raise IndexError("linkedlist: attempted remove, value found, but broke during mending of linkedlist")
                
                prevnode.set_next(nextnode)

                if not isinstance(nextnode, doublenode):
                    raise IndexError("linkedlist: attempted remove, value found, but broke during mending of linkedlist")
